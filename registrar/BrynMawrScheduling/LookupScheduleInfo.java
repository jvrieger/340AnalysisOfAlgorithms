...
class LookupScheduleInfo {
    ...
    int roomIdCount = 0;
    int teacherIdCount = 0;
    int num = 0;
    HashMap<String, Department> allDepartments;
     ...
     */
    public int parseNumRooms(String line) {
        //line is of the format "Rooms\t#"
	    String[] tokens = line.split("\t", 2);
        this.allRooms = new Room[Integer.parseInt(tokens[1])];
        return Integer.parseInt(tokens[1]);
    ...
    public Room parseRoom(String line) {
        ...
        //return Room object
        Room toReturn = new Room(tokens[IDIDX], Integer.parseInt(tokens[CAPIDX]), roomIdCount);
        allRooms[roomIdCount] = toReturn;
        roomIdCount++;
        return toReturn;
    }
     ...
     */
    public int parseNumCourses(String line) {
        //line is of the format "Classes\t#"
	    String[] tokens = line.split("\t", 2);
        this.allCourses = new Course[Integer.parseInt(tokens[1])];
        return Integer.parseInt(tokens[1]);
    }
     ...
     */
    public int parseNumTeachers(String line) {
        //line is of the format "Teachers\t#"
	    String[] tokens = line.split("\t", 2);
        this.allTeachers = new Teacher[Integer.parseInt(tokens[1])];
     ...
     * @param line One line from the constraints file
     */
    public void parseTeacher(String line, boolean[] teacherCreated, Teacher[] allTeachers) {
        ...
        //if department is in allDepartments, add this department to the course
        //add the course to the department
        if(allDepartments.containsKey(dep)){
            
            if(this.findCourse(courseId)!=-1){
                allCourses[this.findCourse(courseId)].addDepartment(allDepartments.get(dep));
                allDepartments.get(dep).addCourse(allCourses[this.findCourse(courseId)]);
            }
            else{
                allCourses[courseIdCount] = new Course(courseId, 1, courseIdCount);
                allCourses[courseIdCount].addDepartment(allDepartments.get(dep));
                courseIdCount++;
                allDepartments.get(dep).addCourse(allCourses[this.findCourse(courseId)]);
            }
        }
        else{
            //else make the department
            //add to allDeparmtnets
            //add the course to the department
            Department toAdd = new Department(dep);
            for(int i = 3; i<tokens.length; i++){
                toAdd.addRoom(allRooms[this.findRoom(tokens[i])]);
            }
            if(this.findCourse(courseId)!=-1){
                toAdd.addCourse(allCourses[this.findCourse(courseId)]);
            }
            else{
                allCourses[courseIdCount] = new Course(courseId, 1, courseIdCount);
                courseIdCount++;
            ...
            allDepartments.put(dep, toAdd);
            allCourses[this.findCourse(courseId)].addDepartment(toAdd);
        }
        /* for (int i = 0; i < numCourses; i++) {
            System.out.println(allCourses[i]);

        } */
        //go through the rooms and put in department
        if (findTeacher(teacherId) > -1) {
            //if teacher already made simply update their class list
            this.allTeachers[findTeacher(teacherId)].addClass(courseId);
            ...
            this.allTeachers[teacherIdCount] = new Teacher(teacherId, courseId, teacherIdCount);
            teacherCreated[teacherIdCount] = true;
            teacherIdCount++;
            //System.out.println(teacherIdCount);
        }
    ...
    public Teacher[] readConstraints(String filename) throws FileNotFoundException {
        ...
        for(int i = 0; i < this.numCourses; i++) {
            String line = input.nextLine(); //temp string to be parsed
            System.out.println("parse teacher called: " + i);
            parseTeacher(line, teacherCreated, this.allTeachers); //call an updater method on line for allTeachers
        }
        ...
        for (int i = 0; i < this.allTeachers.length; i++) {
           // System.out.println("HEREEEE");
          /*   for(int p = 0; p< allTeachers.length; p++) {
                System.out.println(this.allTeachers[p]);
            } */
            //System.out.println(this.allTeachers.length);
            ArrayList<Integer> classes1 = this.allTeachers[i].getClasses();
            //System.out.println(classes1);
            for (int k = 0; k < classes1.size(); k++) {
                for(int j = 0; j < classes1.size(); j++){
                    conflicts[findCourse(classes1.get(j))][findCourse(classes1.get(k))] = numStudents + INFINITY;
                    conflicts[findCourse(classes1.get(k))][findCourse(classes1.get(j))] = numStudents + INFINITY;
                }
            }
        }
    ...
    public int findTeacher(int id) {
        for (int i = 0; i < this.numTeachers; i++) {
            //System.out.println(this.allTeachers[i]);
            if (this.allTeachers[i] != null) {
                if (this.allTeachers[i].getTeacherId() == id) {
                    //System.out.println(i);
                    System.out.println("DUPLICATE " + id);
                    System.out.println("AND : " + this.allTeachers[i].getTeacherId());
                    return i;
                }
            }
    ...
    public int findRoom(String id) {
        for (int i = 0; i < this.numRooms; i++) {
            if (this.allRooms[i] != null) {
                if (this.allRooms[i].getId().compareTo(id) == 0) {
                    return i;
                }
            }
    ...
    public Course[] parseCourses(String line, boolean[] courseCreated) {
        ...
        for (int i = 0; i < courseList.length; i++) { 
            ...
            if (this.allCourses == null || findCourse(Integer.parseInt(courseTokens[i])) < 0) { //create new course
                ...
                courseList[i] = this.allCourses[courseIdCount-1]; //add to temp return array courseList
